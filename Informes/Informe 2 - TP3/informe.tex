...
Para la resolución del sistema (2) se necesita usar Gauss con pivoteo ya que la matriz no es estrictamente diagonal dominante. Por inspección visual en el segundo reglón tenemos que el valor absoluto del elemento de la diagonal es 1 y la suma de los valores absolutos de los demás elementos es 8, por ende no es E.D.D. Teorema 6.18 \cite[p.~398]{burden} . Además podemos observar que no es simétrica definida positiva. Somos capaces de verificarlo de dos maneras; la primera es que vemos un elemento negativo en la diagonal Teorema 6.21 \cite[p.~401]{burden} y la segunda manera es verificarlo con el siguiente algoritmo \hyperref[alg10]{es\_SDP} .\\
